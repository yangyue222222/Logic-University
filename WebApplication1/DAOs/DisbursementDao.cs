...
namespace WebApplication1.DAOs
{
    public class DisbursementDao
    {
        ...
        public static void RemoveDisbursementDetails(List<DisbursementDetail> details)
        {
            using(var ctx = new UniDBContext())
            {
                List<int> detailIds = details.Select(d => d.DisbursementDetailId).ToList();
                List<DisbursementDetail> disDetails = ctx.DisbursementDetails.Where(dis => detailIds.Contains(dis.DisbursementDetailId))
                    .ToList();

                foreach(var d in disDetails)
                {
        ...
        public static List<DisbursementDetail> GetPreparedDisbursementByRequestIds(List<int> requestIds)
        {
            using(var ctx = new UniDBContext())
            {
                List<DisbursementDetail> disbursementDetails = ctx.DisbursementDetails.Include("Disbursement").Include("Disbursement.Request").Include("Item")
                    .Where(dd => dd.Disbursement.Status == (int)DisbursementStatus.Prepared && requestIds.Contains(dd.Disbursement.Request.RequestId))
                    .ToList();
